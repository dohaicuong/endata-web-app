...
import { Container, Paper, makeStyles, LinearProgress, Typography, Button, Grid } from '@material-ui/core'
...
import { LoginFormMutation, UserCredential } from './__generated__/LoginFormMutation.graphql'
              ...
              setTimeout(() => {
                loginMutate({
                  ...
                  onCompleted: (res, errors) => {
                    ...
                    if (res.userAuthenticateAllServer && res.userAuthenticateAllServer[0]) {
                      const { accessToken, refreshToken, user } = res.userAuthenticateAllServer[0]

                      const availableRoutes = user?.privileges?.menus?.map((menu) => menu?.nodeName.toLowerCase()) ?? []
                      const [rootRoute] = ['dashboard', 'communicationtasks', 'media', 'claims']
            ...
            <Form>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} className={classes.loginBtnRow}>
                  <Button variant='contained' color='primary' type='submit' disabled={isOnFly}>
                    Login
                  </Button>
                </Grid>
              </Grid>
